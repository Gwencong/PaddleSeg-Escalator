...
sys.path.append(Path(__file__).parent.absolute().__str__())  # to run '$ python *.py' files in subdirectories

from utils import colorstr, file_size, set_logging
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f'\n{prefix} ONNX model file size: {file_size(onnx_path):.1f} MB')
        ...
        shapes = [network.get_input(i).shape for i in range(network.num_inputs)]
        isdynamic = np.min(shapes)<0
        if isdynamic or opt.dynamic:
            profile = builder.create_optimization_profile()     
            profile.set_shape("x", (bs, 3, 256, 256), (bs, 3, 640, 640), (bs, 3, 1280, 1280))
...
# python3 segmentation_inference/export_TRT.py --onnx out/export/pp_liteseg/pp_liteseg.onnx